...
def lot_kb(data:dict) -> InlineKeyboardMarkup:
    ...
    kb.button(text='До конца аукциона ' + minutes_to_hours_and_minutes(data.get('time_in_minutes')))
    if data.get('current_rate'):
        kb.button(text=f'Текущая ставка: {data.get('current_rate')}')
    else:
        kb.button(text='Ставок пока нет')
