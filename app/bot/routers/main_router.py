        ...
        async with async_session_maker() as session:
            ...
            if user_id in admins:
                values = UserModel(
                    ...
                    phone_number=None,
                    user_enter_fio=None,
                    verification_status=User.VerificationCode.Verified,
                    role=User.Role.Admin,
                )
                await UserDAO.add(session=session, values=values)
                await message.answer(
                    "Привет администрации", reply_markup=MainKeyboard.build_main_kb(user_role=user_info.Role.admin)
                )
                return
    ...
    try:
        ...
        async with async_session_maker() as session:
            ...
            await UserDAO.add(session=session,
                              values=user)
        msg_for_admins = f'Новый пользователь:\nИмя в тг:**{message.from_user.first_name}**\ntelegram_id:`{message.from_user.id}`\nНомер телефона:{data.get('phone_number')}\nУказанное пользователем фио:{fio}'
        for admin in admins:
            await bot.send_message(admin.telegram_id,msg_for_admins,reply_markup=verified_user())
